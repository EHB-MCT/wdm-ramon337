    ...
    <div className="wizard-container">
      ...
      {step === 2 && (
        <div>
          ...
          <input type="number" name="commuteTime" value={formData.commuteTime} onChange={handleChange} style={{ width: "80px" }} />
          ...
          <input type="range" name="flexibility" min="1" max="10" value={formData.flexibility} onChange={handleChange} />
      ...
      {step === 3 && (
        <div>
          ...
          <p style={{ fontSize: "0.9rem", color: "#666", marginBottom: "15px" }}>Voeg je activiteiten toe zodat we ze kunnen inplannen.</p>

          <div style={{ background: "#f9f9f9", padding: "15px", borderRadius: "8px", marginBottom: "20px" }}>
            <label>Naam activiteit</label>
            <input type="text" name="name" placeholder="bv. Tennis, Gamen..." value={newHobby.name} onChange={handleHobbyChange} style={{ marginBottom: "10px" }} />
            
            <input
              type="text"
              name="location"
              placeholder="Address / Village / City"
          ...
          {formData.hobbies.length > 0 ? (
            <ul style={{ listStyle: "none", padding: 0, marginBottom: "20px" }}>
              {formData.hobbies.map((h, index) => (
                ...
                >
                  <div>
                    <strong>{h.name}</strong> ({h.frequency}x/week, {h.duration}u)
                    {h.location && <div style={{fontSize: "0.8em", color: "#666"}}>📍 {h.location}</div>}
                  </div>
                  <button onClick={() => removeHobby(index)} style={{ background: "transparent", color: "red", border: "none", cursor: "pointer", fontWeight: "bold" }}>
                    ✕
                  </button>
                </li>
              ))}
